...
def load_vectorstore(_embeddings):
    if not os.path.exists("chroma_db"):
        vectorstore = Chroma.from_documents(
            documents=splits, embedding=_embeddings, persist_directory="chroma_db"
        )
    else:
        vectorstore = Chroma(
            persist_directory="chroma_db", embedding_function=_embeddings
        )
    return vectorstore
...
3. Do not explicitly separate the answer into "document information" and "general knowledge".
...
if question:
    with st.spinner("Generating response..."):
        ...
        weak_retrieval = len(context.strip()) < 200
        ...
        if weak_retrieval:
            ...
            st.warning(
                "Relevant document context not found. "
    ...
    for i, doc in enumerate(docs):
        ...
        st.write(f"Chunk {i+1} | File: {source} | Page: {page}")
